...
(function (args) {
    ...
    if (has('loader-configurable')) {
        ...
        if (has('loader-config-attribute') && has('host-browser')) {
            Array.prototype.slice.call(document.getElementsByTagName('script'), 0).forEach(function (script) {
    ...
    function getModuleInformation(moduleId, referenceModule) {
        ...
        var matches = Object.keys((config && config.pkgs || {})).filter(function (pkg) { return (moduleId + '/').indexOf(pkg + '/') === 0; }).sort(function (a, b) { return a.length > b.length ? -1 : 1; });
        if (matches.length) {
